...
numTree'' tree = newTree
  where
    (newTree, table)                               = traverseConvert (tree,[])
    traverseConvert (Nil, table)                   = (Nil, table)
    traverseConvert ((Node val left right), table) = ((Node (lookup' val rightTable) newLeft newRight), rightTable)
      where
...
-- Node 3 (Node 1 (Node 0 Nil Nil) (Node 2 Nil Nil)) (Node 4 Nil Nil)

-- Post-order-Traversierung
numTreePostO'' :: (Show a, Eq a) => Tree a -> Tree Int
numTreePostO'' tree = newTree
  where
    (newTree, table)                               = traverseConvert (tree,[])
    traverseConvert (Nil, table)                   = (Nil, table)
    traverseConvert ((Node val left right), table) = ((Node (lookup' val (update val rightTable)) newLeft newRight), (update val rightTable))
      where
        (newLeft, leftTable)   = traverseConvert (left, table)
        (newRight, rightTable) = traverseConvert (right, leftTable)

-- *Main> numTreePostO'' tree
-- Node 4 (Node 2 (Node 0 Nil Nil) (Node 1 Nil Nil)) (Node 3 Nil Nil)

tree = (Node 5 (Node 3 (Node 2 Nil Nil) (Node 6 Nil Nil)) (Node 7 Nil Nil))
